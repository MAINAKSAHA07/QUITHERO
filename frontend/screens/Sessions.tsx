  ...
  return (
    <div className="min-h-screen min-h-[100dvh] pb-24 bg-background">
      <TopNavigation
        left="menu"
        center="Program"
        right={<button onClick={loadSessions} className="p-2 rounded-full hover:bg-muted touch-target"><RefreshCw className="w-5 h-5 text-foreground" /></button>}
      />

      <div className="app-container px-3 sm:px-4 pt-4 sm:pt-6 pb-8">
        {/* Progress Header */}
                ...
                >
                  <CardContent className="p-3 sm:p-4 flex items-center gap-3">
                    <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                      ...
                      {isCompleted ? <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-success" /> :
                       d.isLocked ? <Lock className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-muted-foreground" /> :
                       <span className="text-primary font-bold text-xs sm:text-sm">{d.day.day_number}</span>}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-foreground text-xs sm:text-sm truncate">{d.day.title}</p>
